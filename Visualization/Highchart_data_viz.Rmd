...
knitr::opts_chunk$set(echo = TRUE, warning=FALSE, message=FALSE) # We modified for the environment.
``` 
...
```{r}
survey18 <- read_csv("~/Downloads/stack-overflow-2018-developer-survey/survey_results_public.csv")
glimpse(survey18)
```
...
This function creates a Highchart chart using htmlwidgets. The widget can be rendered on HTML pages generated from R Markdown, Shiny, or other applications.Similarly, Once the `highchart()` function is defined further highchart elements can be added on top of it.
...
survey18 %>% 
  ...
  filter(!is.na(Age),
         ...
         ConvertedSalary > 0) %>% 
  dplyr::select(Gender, Country, Age, ConvertedSalary) %>% 
